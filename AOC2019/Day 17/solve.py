...
def run_robot(mode=1, inputs: list=None):
...
def solve_part_1():
    output = run_robot()
    view = ''.join(map(chr, output)).splitlines()
    HEIGHT = len(view)
    WIDTH = len(view[0])
    ...
    for y in range(1, HEIGHT-1):
        for x in range(1, WIDTH-1):
            if view[y][x] == '#' and view[y-1][x] == '#' and view[y][x-1] == '#' and view[y][x+1] == '#' and view[y+1][x] == '#':
                total += x * y
    return total
...
def solve_part_2():
    ...
    vid = 'n\n'
    output = run_robot(mode=2, inputs=[*map(ord, main), *map(ord, A), *map(ord, B), *map(ord, C), *map(ord, vid)])
    return output[-1]
